...
public class ArchiveSearcher implements Startable {
    ...
    /*
     * Searches the database for conversations that appear suspect.
     */
    public Collection<Conversation> searchDatabaseForSuspects(java.util.Date startDate,java.util.Date endDate,long maxMessageCount,long maxTime)
    {
        CachedPreparedStatement statement = new CachedPreparedStatement();
        ...
        query.append("SELECT DISTINCT conversationID,startDate\n" +
                "FROM\n" +
                "(\n" +
                "SELECT DISTINCT ofMessageArchive.conversationID,ofConversation.startDate\n" +
                "FROM ofMessageArchive\n" +
                "INNER JOIN ofKeywords on ofMessageArchive.body like CONCAT('%', ofKeywords.Keyword, '%')\n" +
                "LEFT OUTER JOIN ofConversation on ofConversation.conversationID = ofMessageArchive.conversationID\n");

        //"INNER JOIN ofKeywords on ofMessageArchive.body like CONCAT('%', ofKeywords.Keyword, '%')\n" +
        //"INNER JOIN ofKeywords on ofMessageArchive.body like '%' + ofKeywords.Keyword + '%'\n" +

        if(maxMessageCount !=0)
        {
            query.append("UNION\n" +
                    "SELECT DISTINCT ofConversation.conversationID,ofConversation.startDate\n" +
                    "FROM ofConversation\n" +
                    "WHERE ofConversation.messageCount > ?\n" +
                    "AND ofConversation.room is null\n");
            statement.addLong(maxMessageCount);
        }
        ...
        if(maxTime != 0)
        {
            query.append("UNION\n" +
                   "SELECT DISTINCT ofConversation.conversationID,ofConversation.startDate\n" +
                   "FROM ofConversation\n" +
                   "WHERE (ofConversation.lastActivity - ofConversation.startDate) > ?\n" +
                   "AND ofConversation.room is null\n");
            statement.addLong(maxTime);
        }
        ...
        if(startDate != null)
        {
            query.append("AND output.startDate >= ?\n");
            statement.addLong(startDate.getTime());
        }

        if(endDate != null)
        {
            query.append("AND output.startDate <= ?\n");
            statement.addLong(endDate.getTime());
        }

        query.append("ORDER BY output.startDate DESC");

        statement.setSQL(query.toString());

        try {
            con = DbConnectionManager.getConnection();
            pstmt = DbConnectionManager.createScrollablePreparedStatement(con, statement.getSQL());
            statement.setParams(pstmt);
